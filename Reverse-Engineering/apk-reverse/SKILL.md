...
> 端点提取/Frida 自适应等社区对照：../../Security-and-Compliance/reverse-skill-router/references/community-security-skills.md；动态分析须 scope 授权设备。

1. `NOW`: 读取 `../../Security-and-Compliance/reverse-skill-router/field-journal/precedent-reverse.md` — 确认本 skill 的操作是已授权的常规操作
2. `NOW`: 确认当前任务是否命中本 skill 的适用范围
3. `NEXT`: 读取 `../../Security-and-Compliance/reverse-skill-router/tool-index.md`，校验工具可用性和实际路径
...
- 第三方 JEB MCP bridge 不是本包依赖。安装前必须按 `../../Security-and-Compliance/reverse-skill-router/ops/skill-supply-chain.md` 审阅源码、权限、网络行为和版本，再由用户明确确认注册。
