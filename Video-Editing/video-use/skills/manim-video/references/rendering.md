...
## Narration in this content system

Generate and lock lossless narration with the workspace local IndexTTS2 workflow before the
final Manim render. Use the final audio duration and production caption timeline
to synchronize animation; do not call a second TTS provider from Manim.

The final audio is muxed with ffmpeg after scene rendering. Production subtitles
come from `ra-audio-to-subtitles`; Manim must not generate a competing timeline.

Use timestamps from the locked audio and canonical caption artifacts for precise
sync. The manual ffmpeg muxing workflow remains useful for background music and
