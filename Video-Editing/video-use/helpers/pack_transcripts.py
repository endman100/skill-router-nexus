"""Pack all word-level ASR transcripts in <edit>/transcripts/ into readable markdown.
...
def join_tokens(parts: list[str]) -> str:
    """Join ASR tokens without inserting spaces between Chinese characters."""
    output = ""
    ...
    for part in parts:
        ...
        previous = output[-1]
        first = part[0]
        if (is_cjk(previous) and is_cjk(first)) or first in closing_punctuation or previous in opening_punctuation:
            output += part
        else:
            output += " " + part
    return output
...
def group_into_phrases(
    ...
    Normalized `words` entries have types 'word', 'spacing', or 'audio_event'.
...
def render_markdown(entries: list[tuple[str, float, list[dict]]], silence_threshold: float) -> str:
    ...
    for name, duration, phrases in entries:
        ...
        for p in phrases:
            ...
            if spk is not None:
                # Some providers return IDs like "speaker_0" — strip the prefix.
                spk_str = str(spk)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Pack word-level ASR transcripts into takes_packed.md")
