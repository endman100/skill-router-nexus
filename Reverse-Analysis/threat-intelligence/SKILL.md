...
> 分類整合：此項目來自 `reverse-skill-router` 套件。本文中的 `<skill-root>` 與「包根目錄」指 `../reverse-skill-router`；共享資源及其他專項 skill 以該目錄為準。

# Threat Intelligence & Public-Source OSINT

## ACTION REQUIRED（读完后立刻执行）

1. `NOW`: 读取 `../reverse-skill-router/ops/scope-contract.md`，确认公开来源、目标实体、时间窗与交付用途。
2. `NOW`: 仅在需要操作先例时读取 `../reverse-skill-router/field-journal/precedent-pentest.md`。先例不能授予权限。
3. `NOW`: 写出可证伪的情报问题，以及必须独立核验的候选结论。
4. `NEXT`: 读取 `../reverse-skill-router/tool-index.md`。需要公开 X 数据时检查 `xquik-mcp`。
