...
本入口负责路由同层 `Reverse-Analysis/` 分类中的逆向工程 skill；各 specialist skill 都是分类根目录下的直接子目录，内含 `SKILL.md` 描述其适用场景、工具链和工作流程。本目录只保留总控脚本与共用资源，不重复注册 specialist skill。
...
本分类会持续增长。发现新的同层 skill 目录时，读它的 `SKILL.md` 就能快速了解用途。
