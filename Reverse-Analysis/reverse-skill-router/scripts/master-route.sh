...
"$PYTHON" - "$CONFIG_PATH" "$SKILLS_ROOT" "$CATEGORY_ROOT" "$CATEGORY_NAME" "$PROJECT_ROOT" "$OUT_DIR" "$HINT" <<'PY'
import datetime
import json
import pathlib
import re
...
config_path, skills_root_raw, category_root_raw, category_name, project_root_raw, out_dir_raw, hint = sys.argv[1:]
skills_root = pathlib.Path(skills_root_raw)
category_root = pathlib.Path(category_root_raw)
...
skill_path = category_root / pathlib.PurePosixPath(primary_path)
if not skill_path.is_file():
    print(f"ERROR: PRIMARY skill missing: {skill_path}", file=sys.stderr)
    raise SystemExit(2)

out_dir.mkdir(parents=True, exist_ok=True)
secondary = [f"{category_name}/{routes[item]['skill']}" for item in selected_order if item != primary]
lines = [
    ...
    f"- primary_skill: {category_name}/{primary_path}",
    ...
    "## MUST open next",
    "",
    f"1. {category_name}/reverse-skill-router/MASTER-ROUTING.md",
    f"2. {category_name}/{primary_path}",
    "",
    "## Notes",
]
...
print(f"PRIMARY -> {category_name}/{primary_path}")
