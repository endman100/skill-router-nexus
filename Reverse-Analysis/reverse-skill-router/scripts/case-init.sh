...
if [[ -f "$SCRIPT_DIR/master-route.sh" ]]; then
  ...
  if [[ -f "$ROUTE_TMP/route-scope.md" ]]; then
    ...
    if printf '%s\n' "$rt" | grep -Eq '^[[:space:]]*-[[:space:]]*primary_skill:[[:space:]]*(skills|Reverse-Analysis)/'; then
      primary="$(printf '%s\n' "$rt" | grep -E '^[[:space:]]*-[[:space:]]*primary_skill:[[:space:]]*(skills|Reverse-Analysis)/' | head -1 | sed -E 's@.*(skills|Reverse-Analysis)/@@' | tr -d '\r')"
    fi
