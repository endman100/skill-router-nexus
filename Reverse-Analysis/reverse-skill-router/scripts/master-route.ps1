...
$categoryRoot = $packageRoot
$categoryName = Split-Path -Leaf $categoryRoot
...
$skillAbs = Join-Path $categoryRoot ($primaryPath -replace '/', [IO.Path]::DirectorySeparatorChar)
...
[void]$sb.AppendLine(("- primary_skill: {0}/{1}" -f $categoryName, $primaryPath))
...
foreach ($d in $uniq) {
    if ($d -ne $primary) { [void]$sec.Add(("{0}/{1}" -f $categoryName, $cfg.routes.$d.skill)) }
}
...
[void]$sb.AppendLine('')
[void]$sb.AppendLine(("1. {0}/reverse-skill-router/MASTER-ROUTING.md" -f $categoryName))
[void]$sb.AppendLine(("2. {0}/{1}" -f $categoryName, $primaryPath))
[void]$sb.AppendLine('')
...
Write-Host ("PRIMARY -> {0}/{1}" -f $categoryName, $primaryPath) -ForegroundColor Green
