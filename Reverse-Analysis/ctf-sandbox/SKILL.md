...
1. `NOW`: 跑 `../reverse-skill-router/scripts/case-init.ps1`；`auth.status=granted` 前禁止对真实外网 ACT。竞赛/靶场用 `-NetworkProfile lab` 或 `offline`。
2. `NOW`: 打开包根下的 `../ctf-sandbox-orchestrator/SKILL.md`，按它的 sandbox 假设继续。
