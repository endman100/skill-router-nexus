...
| 2 | `Agent-Plan-Review/` | Agent Plan Review | ⚠️ 計畫完成後、執行前必查此分類。涵蓋多視角計畫審查（CEO/設計/工程）並生成正式審查報告與自動審查管道。適用於計畫草稿需要壓力測試或多視角交叉驗證的場景。 |
| 3 | `Agent-Execute/` | Agent Execute | 涵蓋計畫實際執行、子代理平行派遣、子代理驅動開發、以及執行中的檢查點儲存與恢復。適用於 Agent 需要將計畫付諸行動、平行派發子任務、或中途暫停與繼續的場景。 |
...
| 7 | `Coding/` | Coding | 此分類涵蓋程式開發工作流程中的核心實務，包括測試驅動開發、系統化除錯、程式碼審查、Git 分支管理、Web 應用測試、MCP 伺服器建置、以及程式碼執行與分析。適用於需要遵循工程最佳實務進行軟體開發的場景。 |
