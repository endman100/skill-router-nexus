...
class UnmeasuredLayoutTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config.update(canvas_height_px=360, body_margin_px=10, expression_column_width_px=240)
    ...
    def test_wide_body_expands_without_squeezing(self):
        ...
        im, data = self.render()
        self.assertEqual(data["body_views"]["back"]["placed_box"][2] - data["body_views"]["back"]["placed_box"][0], 680)
        self.assertEqual(im.width, (680 + 32) * 3 + 240)
        self.verify(im, data)
    ...
    def test_normal_must_be_unchanged_anchor(self):
    ...
    def test_invalid_dimensions_rejected(self):
        for key, value in (("canvas_height_px", True), ("body_margin_px", 200), ("panel_side_padding_px", -1), ("expression_column_width_px", 0)):
    ...
    def test_expression_gap_detected(self):
        im, data = self.render()
        data["expressions"]["angry"]["cell"][1] += 1
        with self.assertRaisesRegex(AssertionError, "expression gap"):
            self.verify(im, data)
