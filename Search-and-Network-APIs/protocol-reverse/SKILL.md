...
> 分類整合：此項目來自 `reverse-skill-router` 套件。本文中的 `<skill-root>` 與「包根目錄」指 `../../Security-and-Compliance/reverse-skill-router`；共享資源及其他專項 skill 以該目錄為準。

# Protocol Reverse Engineering

## ACTION REQUIRED（读完后立刻执行）

1. `NOW`: 读取 `../../Security-and-Compliance/reverse-skill-router/field-journal/precedent-reverse.md` — 确认授权与常规操作边界
2. `NOW`: 确认任务是否为**协议/流量/序列化格式**逆向（非纯 Web 参数签名 → 转 `js-reverse/`）
3. `NOW`: 若有目标网络交互 → `../../Security-and-Compliance/reverse-skill-router/scripts/case-init.ps1` 完成 scope；`auth` 未 granted 禁止对目标 ACT
4. `NEXT`: 读取 `../../Security-and-Compliance/reverse-skill-router/tool-index.md`；缺工具 bootstrap（tshark/wireshark 等可能需手动）
...
- 相关：`../../Security-and-Compliance/reverse-skill-router/ida-reverse/` `../../Security-and-Compliance/reverse-skill-router/js-reverse/` `../../Security-and-Compliance/reverse-skill-router/firmware-pentest/` `../../Security-and-Compliance/reverse-skill-router/pentest-tools/`
