...
- [ ] 至少提供兩個可直接執行的 Workflow：推薦最小流程，以及真正不同的支援模式或實用組合；輸入需有標示並連接最終輸出。
- [ ] 依目標 Schema 驗證並實際 Queue 每個隨附 Workflow，確認最終輸出；不能用只改參數的重複流程滿足數量。
...
- [ ] 每個隨附 Workflow 都能從 `example_workflows` 載入、成功 Queue，並產生文件描述的最終輸出。
...
- [ ] README 遵循 [readme-template.md](readme-template.md)，涵蓋原始 Repository、專案目標與特色、包裝方法、安裝與使用、Node、Workflow，以及可重現的官方／ComfyUI 實作結果。
