 ...
 * composer references (`hero.jpg`, `about.jpg`, `lab-1.jpg`, …) so
 ...
 * slots by .jpg filename; we honor that by writing `<basename>.svg`
 * AND a `<basename>.jpg.svg` symlink-style fallback. Most static
 ...
 * a directory of `.svg` files OR rename the SVGs to `.jpg` (some
 * browsers honor extensionless content-sniffing).
 *
 * For the most reliable result, write BOTH:
 *   - `<id>.svg`   — clean, editable
 *   - `<file>`     — same SVG content under the .jpg filename so the
 *                    composer's `<img src='./assets/<id>.jpg'>` works
...
export async function writePlaceholders(outDir: string): Promise<string[]> {
  ...
  for (const slot of manifest.slots) {
    ...
    const assetPath = resolve(outDir, slot.file);
    await writeFile(svgPath, svg, 'utf8');
    await writeFile(assetPath, svg, 'utf8');
    written.push(svgPath, assetPath);
  }
  return written;
}
...
async function main(): Promise<void> {
  ...
  const jpgs = written.filter((p) => p.endsWith('.jpg')).length;
  const svgs = written.filter((p) => p.endsWith('.svg')).length;
  console.log(`✓ wrote ${jpgs} jpg-named placeholders + ${svgs} svg files into ${out}`);
