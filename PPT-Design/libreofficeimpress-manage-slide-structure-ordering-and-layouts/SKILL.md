...
- `Images/slide_thumbnail_selected.jpg`: use when the main need is to confirm that the intended slide thumbnail is selected before a structural action.
- `Images/slide_layout_control.jpg`: use when a thumbnail context menu is open and you need to choose a layout or another slide-level control from that selected slide.
- `Images/slide_structure_result.jpg`: use when the action is complete and the main canvas should now show the result that needs verification.
