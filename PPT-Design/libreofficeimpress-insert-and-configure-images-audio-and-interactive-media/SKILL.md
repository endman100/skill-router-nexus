...
Current image cards:
- `Images/media_insert_surface.jpg`: selected slide plus empty content placeholder ready to start image insertion
- `Images/media_result_state.jpg`: system `Insert Image` chooser after starting insertion; despite the filename, this is not a finished on-slide result
- `Images/media_interaction_controls.jpg`: open `Interaction` dialog for configuring click behavior on an already selected media object
...
3. If the system chooser opens, use the state represented by `Images/media_result_state.jpg` only for selecting the requested image file and confirming with `Open`.
