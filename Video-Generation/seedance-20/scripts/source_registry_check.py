...
def main() -> int:
    ...
    if api_status.exists():
        ...
        if anchor:
            anchor_age = (date.today() - anchor).days
            if anchor_age > 30:
                errors.append(f"api-status.md last_verified is {anchor_age} days old")
            elif anchor_age > 14:
                warnings.append(f"api-status.md last_verified is {anchor_age} days old")
