...
| Runway | Runway web/API/MCP workflow with `seedance2`, `seedance2_fast`, or `seedance2_mini`, hosted uploads, and Runway plan/region constraints. | model lane, duration, ratios, resolution tier, price, plan, region, SDK field support, audio/reference combination rules |
...
Runway's model, pricing, changelog, input, and help docs are the current sources for its `seedance2`, `seedance2_fast`, and `seedance2_mini` surfaces, hosted uploads, duration, reference-count rules, resolution tiers, and SDK caveats. The former Seedance-specific developer guide returned 404 on 2026-07-27. Do not copy Runway field names into Volcengine examples or vice versa.
