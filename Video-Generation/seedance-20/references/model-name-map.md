...
| `seedance2` / `seedance2_fast` / `seedance2_mini` | Runway API model IDs verified in current model docs on 2026-07-27 | Use only for Runway's API surface. Main, Fast, and Mini have different resolution and pricing lanes; do not substitute them for Volcengine/Doubao model IDs. |
| fal Seedance 2.0 endpoints | fal's hosted Seedance 2.0 surface: `text-to-video`, `image-to-video`, `reference-to-video`, each with a `/fast` tier | Use fal endpoint naming only for the fal surface (verified 2026-07-27). Recheck endpoint IDs, resolution tiers, and per-second pricing live before quoting. Do not substitute for Volcengine, Doubao, or Runway model IDs. |
